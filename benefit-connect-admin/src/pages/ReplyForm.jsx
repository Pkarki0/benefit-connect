  ...
  const fetchInquiry = async () => {
    try {
      ...
      if (response.ok) {
        ...
        setReplyMessage(data.data.replyMessage);
      } else {
  ...
  return (
    <div className="flex flex-col items-center p-6 w-full max-w-4xl mx-auto my-16 space-y-6">
      ...
      <div className="w-full">
        <span className="font-bold text-gray-900">Reply Message: </span>
        <textarea
          ...
          disabled={inquiry.isReplied ? true : false}
        ></textarea>
        <button
          onClick={handleReply}
          disabled={inquiry.isReplied}
          className={`mt-4 py-3 px-6 rounded-lg w-full font-medium focus:outline-none focus:ring-2 focus:ring-blue-400 ${
            inquiry.isReplied
